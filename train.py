...
# Imports category names from 'cat_to_name.json'
...
parser.add_argument('--arch', type=str, default='vgg16', help='Choose architecture (vgg16, densenet121, or alexnet)')
parser.add_argument('--learning_rate', type=float, default=.001, help='Learning rate')
parser.add_argument('--hidden_layer', type=int, default=1024, help='Number of hidden layer units')
parser.add_argument('--gpu', default='gpu', type=str, help='Specify "gpu" to use GPU or "cpu" for CPU')
...
# Define a function to create a custom classifier for the model
...
# Create a model using the specified architecture and classifier
...
def train_model(model=model, trainloader=trainloader, criterion=criterion, optimizer=optimizer, epochs=epochs, gpu=gpu):
    ...
    for e in range(epochs):
        ...
        for images, labels in trainloader:
            ...
            # Zeroing out the optimizer's gradient
            optimizer.zero_grad()
...
# Define a function to check the accuracy on the test dataset
def test_checker(testloader=testloader, gpu=gpu):
    ...
    with torch.no_grad():
        ...
        print('Accuracy of the network on the test dataset: {}%'.format((correct / total) * 100))

# Run the test_checker function to evaluate the accuracy on the test dataset
test_checker(testloader)

# Saving the checkpoint to the specified save_dir path
